    ...
    public class PageBase : System.Web.UI.Page
    {
        ...
        protected bool UserLogin(string strLoginID, string strLoginPwd)
        {
            ...
            try
            {
                ...
                string level = DataSetUtil.RowValue(dsUser, "level", 0).ToString();
                if (level != "1")
                    return false;
                ...
                AuthUser _authUser = new AuthUser(lID, sLoginID, sLoginPwd);
                Session[Constants.SESSION_KEY_USERINFO] = _authUser;

                if (Defines.COOKIE_INUSED)
                {
                    string strCookieData = string.Format("{0};{1};{2};{3}",
                        _authUser.ID,
                        _authUser.LoginID,
                        _authUser.LoginPwd,
                        CurrentDate);
