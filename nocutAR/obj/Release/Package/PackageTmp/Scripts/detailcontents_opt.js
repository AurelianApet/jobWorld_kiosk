...
panel_obj += "					<img class='delete-btn' margin-left='15px' src='img/icon_trash.png' width='18' height='21' border='0' name='image3'>";
panel_obj += "				</td>";
//panel_obj += "				<td width='30%' height='26' align='center'>";
//panel_obj += "					<img class='3d-btn' src='img/cam_icon12_3d.png' width='25' height='21' border='0' style='display:none;'>";
//panel_obj += "				</td>";
panel_obj += "				<td width='30%' height='26' align='center'>";
panel_obj += "					<img class='edit-btn' margin-right='15px' src='img/cam_icon_pen.png' width='18' height='21' border='0' name='image4'>";
panel_obj += "				</td>";
panel_obj += "			</tr>";
panel_obj += "		</table>";
panel_obj += "</div>";
...
$(document).ready(function () {
    ...
    /*비디오파일 처리부분*/
    $("#video_file1").bind("change", function () {
        /*확장자체크*/
        if (!checkImgExtention("video_file1", "video")) {
            alert("파일형식이 바르지 않습니다.");
            return;
        }
        ...
        var file = document.getElementById("video_file1");
        var size = file.files[0].size / 1053317.6;
        /*용량제한*/
        if (size > 50) {
    ...
    $("#three_file1").bind("change", function () {
        /*확장자체크*/
        var imgExtension1 = ($("#three_file1").val().substr($("#three_file1").val().length - 3)).toLowerCase();
        if (imgExtension1 != 'zip') {
            alert("파일형식이 바르지 않습니다.");
            return;
        }
        /***확장자체크***/
    ...
    $("#three_file2").bind("change", function () {
        /*확장자체크*/
        var imgExtension2 = ($("#three_file2").val().substr($("#three_file2").val().length - 3)).toLowerCase();
        if (imgExtension2 != 'zip') {
            alert("파일형식이 바르지 않습니다.");
            return;
        }
        /***확장자체크***/
...
function OnVideoApply1() {
    ...
    if ($("#videorun_rd2")[0].checked == true)
        run_opt = 1;
    if ($("#videorun_rd3")[0].checked == true)
        run_opt = 2;
    if ($("#videorun_rd4")[0].checked == true)
        run_opt = 3;
    ...
    if (run_opt == 3)
    {
        ...
        if (!checkImgExtention("video_file2", "img")) {
            return false;
        }
        var file = document.getElementById("video_file2");
        if (file.files[0] != null) {
            ...
            //                setTimeout(function () {
            $.ajax({
                url: 'PostUpload.aspx?type=30&content_id=' + CONTENT_ID,
                type: "post",
                data: data,
                // cache: false,
                processData: false,
        ...
        addFileName = $("#video_file2").val();
        while (addFileName.indexOf("\\") > -1)
        {
            addFileName = addFileName.substr(addFileName.indexOf("\\") + 1);
        }
    }
    if ($("#video_file1").val().substr(0, 22) == "http://www.youtube.com") {
        var pos = $(objName).position();
        var element = '<div id="temp" style="width: 200px; height: auto; position:absolute"><video width="100%" height="100%" controls><source src="' + $("#video_file1").val() + '"></video>';
        //패널 추가
        element += panel_obj;
        ...
        videoList[index] = { url: $("#video_file1").val(), type: val, run_opt: run_opt, objsize: 0 };
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                //setTimeout(function () {
                    ...
                    videoList[index] = { url: server_url + data, type: val, run_opt: run_opt, objsize: size, addFile: addFileName };

                    //패널사건추가
                    add_DoubleClickEvent();
                    //setMarkerImg(data);
                    $("#video_file1").replaceWith($("#video_file1").clone(true));
                //}, 3000);
